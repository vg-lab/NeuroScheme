...
#include <neuroscheme/RepresentationCreatorManager.h>
#include "Neuron.h"
...
namespace neuroscheme
{
  namespace cortex
  {
    bool DataLoader::loadData( const NeuroSchemeInputArguments& args )
    {
      ...
      if ( args.count( "-bc" ) == 1 )
      {
        ...
        createEntitiesFromNsolColumns(
          neuroscheme::DataManager::nsolDataSet( ).columns( ),
          args.count( "-nm" ) == 0,
          ( args.count( "-cns" ) == 1 ? args.at( "cns" )[0] : std::string( )));
    ...
    {
      auto& _entities = neuroscheme::DataManager::entities( );
      auto& _rootEntities = neuroscheme::DataManager::rootEntities( );
