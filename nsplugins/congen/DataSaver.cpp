...
namespace nslib
{
  namespace congen
  {
    void DataSaver::saveConGenXmlScene(  QWidget * ptr )
    {
      ...
      if ( fileName != "" )
      {
        ...
        for ( auto it = relConnectsTo.begin( ); it != relConnectsTo.end( ); ++it )
        {
          const auto& relParameters = it->second;
          for ( auto it2 = relParameters.begin( ); it2 != relParameters.end( ); ++it2 )
          {
            params.clear();
